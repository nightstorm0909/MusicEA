...
#Basic class for computing, managing, plotting EV3 run stats
class EV_Stats_encoder_decoder:
	def __init__(self):
		self.bestIndividual = []
		self.bestFit=[]
		self.meanFit=[]
		self.stddevFit=[]
		self.en_mutationStrength = []
		self.de_mutationStrength = []
		self.finished_time = 0.0
		self.last_population = None
	...
	def accumulate(self, pop):
		...
		max_fit = pop[0].fit
		max_fit_individ = pop[0]
		en_mut_strength = pop[0].encoder_sigma
		de_mut_strength = pop[0].decoder_sigma

		for p in pop:
			if p.fit > max_fit:
				 max_fit = p.fit
				 max_fit_individ = p
				 en_mut_strength = p.encoder_sigma
				 de_mut_strength = p.decoder_sigma

		self.bestIndividual.append(max_fit_individ)
		self.bestFit.append(max_fit)
		self.en_mutationStrength.append(en_mut_strength)
		self.de_mutationStrength.append(de_mut_strength)
	...
	def print(self, gen=None):
		...
		print('Mean fitness	 : {:6.3f}'.format(self.meanFit[gen]))
		print('Stddev fitness   : {}'.format(self.stddevFit[gen]))
		print('Encoder Mutation Strength: {}'.format(str(self.en_mutationStrength[gen])))
		print('Decoder Mutation Strength: {}'.format(str(self.de_mutationStrength[gen])))
	...
	def plot(self):
		...
		tmp = {}
		for key in self.en_mutationStrength[0]:
			tmp[key] = []
		for i in range(len(self.en_mutationStrength)):
			for key in self.en_mutationStrength[i]:
				tmp[key].append(self.en_mutationStrength[i][key])

		for key in tmp:
			plt.plot(gens, tmp[key])
		
		tmp = {}
		for key in self.de_mutationStrength[0]:
			tmp[key] = []
		for i in range(len(self.de_mutationStrength)):
			for key in self.de_mutationStrength[i]:
				tmp[key].append(self.de_mutationStrength[i][key])
