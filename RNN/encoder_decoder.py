...
class rnnModel:
	...
	def encode(self, seq: np.ndarray) -> np.ndarray:
		...
		'''
		encode = self.rnn.initHidden()

		for i in range(len(seq)):
			_, encode = self.rnn.forward(seq[i].reshape(-1, 1), encode)
			
		return encode

	def decode(self, embedding: np.ndarray, length: int) -> np.ndarray:
		'''
		...
		for i in range(length):
			if i > 0:
				...
				output, hidden = self.rnn.forward(np.zeros((1, 1)), hidden)
			gen.append(output)
